...
Buddhabrot::Buddhabrot():Buddhabrot(vec2<size_t>(500, 500), 4, vec2d(0, 0), 2)
{}

Buddhabrot::Buddhabrot(vec2<size_t> resSize, double plotWidth, vec2d center, double multiplicity): m_pixels(resSize.x, resSize.y), m_multiplicity(multiplicity)
{
...
void Buddhabrot::compute(unsigned int maxIterations, bool antiAliasing)
{
...
void Buddhabrot::render(double gamma, RGBGradient &gradient)
{
	size_t width = m_pixels.getWidth(),
		   height = m_pixels.getHeight();
...
RGBBitmap &Buddhabrot::getResult()
{
	return m_pixels;
}
